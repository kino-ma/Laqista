...
#[derive(Clone, Debug)]
pub struct DewScheduler {
    pub runtime: Arc<Mutex<DewSchedulerRuntime>>,
...
impl DewScheduler {
    ...
    pub async fn clone_inner(&self) -> DewSchedulerRuntime {
        DewSchedulerRuntime::clone_inner(&self.runtime).await
    }
}

#[tonic::async_trait]
impl Scheduler for DewScheduler {
    async fn join(&self, _request: Request<JoinRequest>) -> RpcResult<Response<JoinResponse>> {
        unimplemented!("Dew node does not support join()");
    }
    ...
    async fn report(&self, request: Request<ReportRequest>) -> RpcResult<Response<ReportResponse>> {
        ...
        let mut runtime = DewSchedulerRuntime::clone_inner(&self.runtime).await;
    ...
    async fn deploy(
        ...
        unimplemented!("Dew node does not support deploy()");
    }
